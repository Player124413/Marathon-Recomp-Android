...
VkResult volkInitialize(void)
{
	...
	/* On Android the system Vulkan loader is always libvulkan.so; there is no
	 * versioned libvulkan.so.1 variant.  Load the correct name directly so we
	 * avoid a spurious dlopen failure in logcat and always use the system
	 * (hardware) driver rather than any software fallback. */
